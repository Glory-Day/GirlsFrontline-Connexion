...
namespace Core.Object.Character
{
    ...
    public class BitmapTextParticleSystem : MonoBehaviour
    {
        ...
        private readonly List<PendingSpawn> _pending = new List<PendingSpawn>(128);
        private readonly List<Vector4> _customData = new List<Vector4>(1024);
        ...
        private void LateUpdate()
        {
            ...
            var index = _particleSystem.particleCount;

            var parameters = new ParticleSystem.EmitParams();
            for (var i = 0; i < count; i++)
            {
                parameters.startSize3D = _pending[i].Size3D;
                _particleSystem.Emit(parameters, 1);
            }
            ...
            {
                var j = index + i;
                if ((uint)j < (uint)_customData.Count)
                {
                    _customData[j] = _pending[i].Custom1;
                }
            }
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if ((uint)j < (uint)_customData.Count)
                {
                    _customData[j] = _pending[i].Custom2;
                }
            }
        ...
        protected void Spawn(string message, float? startSize = null)
        {
            try
            {
                ...
                var size = new Vector3(message.Length, 1f, 1f);
                if (startSize.HasValue)
                {
                    size *= startSize.Value * _particleSystem.main.startSizeMultiplier;
                }

                _pending.Add(new PendingSpawn
                {
                    Size3D = size, Custom1 = customData01, Custom2 = customData02
                });
            }
